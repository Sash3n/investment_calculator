...
export function OffshoreAllowance() {
  ...
  const result = useMemo(() => {
    ...
    for (let y = 1; y <= years; y++) {
      ...
      chartData.push({
        year:     y,
        local:    Math.round(localBal),
        offshore: Math.round(offshoreZar),
      });
    }

    const finalLocal    = chartData[chartData.length - 1]?.local    ?? 0;
    const finalOffshore = chartData[chartData.length - 1]?.offshore ?? 0;
    const futureRateFinal = spotRate * Math.pow(1 + zarDepreciation / 100, years);
    const usdFinal = usdInvested * Math.pow(1 + offshoreReturn / 100, years);

    return { chartData, finalLocal, finalOffshore, usdFinal, futureRateFinal, usdInvested };
  ...
  const shareUrl  = buildShareUrl({ zarAmount, spotRate, offshoreReturn, localReturn, zarDepreciation, years } satisfies ShareState);
    ...
    >
      ...
      >
        ...
        <div style={{ color: 'var(--color-text-muted)' }}>
          ...
          R1,000,000/year Single Discretionary Allowance (no SARB approval needed) +
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <InputField label="ZAR amount to invest" value={zarAmount} onChange={setZarAmount} prefix="R" min={1} />
          <InputField label="Current ZAR/USD spot rate" value={spotRate} onChange={setSpotRate} suffix="R/USD" min={1} />
          <InputField label="Offshore return (USD p.a.)" value={offshoreReturn} onChange={setOffshoreReturn} suffix="%" min={0} max={40} />
          <InputField label="Local JSE return (ZAR p.a.)" value={localReturn} onChange={setLocalReturn} suffix="%" min={0} max={40} />
          <InputField label="ZAR depreciation p.a." value={zarDepreciation} onChange={setZarDepreciation} suffix="%" min={0} max={20} />
          <InputField label="Investment horizon" value={years} onChange={setYears} suffix="yrs" min={1} max={40} />
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <StatCard
              label="Offshore final value (ZAR)"
              value={formatRandShort(result.finalOffshore)}
              color={C.cyan}
              icon={<Globe size={16} />}
            />
            <StatCard
              label="Local final value (ZAR)"
              value={formatRandShort(result.finalLocal)}
              color={C.indigo}
            ...
            <StatCard
              ...
              color={advantage >= 0 ? C.emerald : C.amber}
              icon={<DollarSign size={16} />}
            />
            <StatCard
              label={`USD value in ${years}yr`}
              value={`$${Math.round(result.usdFinal).toLocaleString()}`}
              color={C.violet}
              icon={<DollarSign size={16} />}
            />
          </div>
          ...
          <div
            ...
            style={{
              background: advantage >= 0 ? `${C.emerald}11` : `${C.amber}11`,
              border:     `1px solid ${advantage >= 0 ? C.emerald : C.amber}33`,
            }}
          >
            <span style={{ color: 'var(--color-text-muted)' }}>
              {advantage >= 0
                ? `Offshore investing outperforms by ${formatRandShort(advantage)} over ${years} years, driven by ZAR depreciation boosting your USD returns when converted back.`
